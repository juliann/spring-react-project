package com.nadarzy.springreactproject.resource.impl;
...
import com.nadarzy.springreactproject.exception.ApplicationException;
import com.nadarzy.springreactproject.exception.BookNotFoundException;
import com.nadarzy.springreactproject.model.Book;
import com.nadarzy.springreactproject.resource.Resource;
...
public class BookResourceImpl implements Resource<Book> {
  ...
  @Autowired private IService<Book> bookService;
  ...
  public ResponseEntity<Book> save(Book book) {
    ...
    if (book.getId() != null) {
      ...
      savedBook.add(
          linkTo(methodOn(BookResourceImpl.class).findById(savedBook.getId())).withSelfRel());
  ...
  public ResponseEntity<Book> update(Book book) {
    ...
    if (book.getId() == null) {
      ...
      updatedBook.add(
          linkTo(methodOn(BookResourceImpl.class).findById(updatedBook.getId())).withSelfRel());
  ...
  public ResponseEntity<String> invalid() {
    ...
    try {
      jsonObject.put(
          "message", "something is missing, please check everything before sending the request!!!");
